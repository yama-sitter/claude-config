...
- **グループ数が 3 を超えたら** 2 階層上のパスに繰り上げて再グルーピング (3 以下に収束するまで段階的に繰り上げる)
- **リポルートまで繰り上げても 3 以下にならない**稀ケースは、**全体を 1 グループとして扱い** `## Group: <repo root>` セクションで 4 軸並列評価する (分割評価モードは維持、fast path は無効)
...
- **軸タグ (観点タグ)**: 本軸の標準セットから選ぶ
  ...
  - シンプルさ: `YAGNI` / `未使用export` / `早期抽象` / `早期generics` / `1箇所抽象`
