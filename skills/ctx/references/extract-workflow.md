...
**RQ (分析の焦点) is NOT a filter for fact extraction.** Extract ALL observable behaviors and verbatim quotes from the source material, regardless of whether they appear relevant to the RQ. Omitting facts because they seem "off-topic" destroys the data foundation for discovering contexts the RQ did not anticipate.
...
**Structural condition test** for each Background candidate: "Is this condition tied to a specific point in time?"

- No → Background (e.g., "Zero applicants from Hello Work for months" = ongoing structural condition)
- Yes → Events (e.g., "Two staff members quit in December" = time-specific event)
- Uncertain → Apply: "Could this condition be observed as an ongoing state over a period, rather than a one-time occurrence?" Yes = Background, No = Events

Output format:

> **前提条件**（特定時点に紐づかない構造的条件）
> ファクト: A1, A3, ...
...
2. **Data sufficiency**: Sufficient data exists for the analysis focus (defined in setup). Report any areas where data is thin or missing relative to the focus direction
...
When only one case exists, the workflow is identical. Background/Events separation and chronological ordering apply to single cases as well — the structure is useful for the subsequent `organize` step regardless of case count.
